#!/usr/bin/env node
/** Verifies replies point operators at the structured log command and name both artifacts. */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { collect } from '../../src/write-meta.mjs';
import { buildResult, makeRun } from './test-fixtures.mjs';

test('successful replies use the log command instead of a raw file path', () => {
  const dir = makeRun({
    args: ['exec', '--json'],
  ...
  assert.ok(reply.includes(`Log: codex-bridge log ${dir}`));
  assert.doesNotMatch(reply, /raw\.log/);
});

test('failed replies report events and stderr sizes plus the log command', () => {
  const dir = makeRun({
    args: ['exec', '--json'],
    events: [],
