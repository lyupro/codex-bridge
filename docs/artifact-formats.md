...
| `stderr_bytes` | Размер `stderr.log` в байтах. В норме ноль: вне протокола CLI молчит. |
...
- `events.jsonl` — поток событий CLI под `--json`, единственный источник вердикта `LIMIT`, расхода
  и идентификатора сессии; `stderr.log` — то, что CLI говорит вне протокола, в норме пуст. Человек
  читает прогон командой `codex-bridge log <прогон>`, которая рендерит события по требованию.
...
- `raw.log` в прогонах до 0.1.5 содержал оба потока вперемешку. Новые прогоны его не пишут:
