...
5. **Результат отсутствует или не заполнен.** Сначала в одной строке ищется одновременно
...
6. **Ненулевой exit при заполненном результате.** Это `FAIL`. Наличие JSON не отменяет сбой
   процесса.
7. **Красная verification у build.** `verify_passed === false` даёт `FAIL`, даже если изменения
...
8. **Объявленный исход build.** Исполнитель сам сообщает в обязательном поле `outcome`, сделана
...
9. **Покрытие scout.** Для нескольких извлечённых вопросов требуются отдельные ответы,
...
10. **Правки build вне scope.** Из delta дерева сначала вычитаются `environmentPaths`, затем
...
11. **Соответствие отчёта build фактической работе.** `changes[].file` сначала сравниваются с
...
12. **Успех.** Совпадение только с накопленной работой возвращает `OK` и `carried: true`; обычное
...
невыполнимой задачи, получал `OK`) закрыта не здесь, а объявленным исходом: проверка 8 краснеет по
