...
7. **Результат отсутствует или не заполнен.** `FAIL` с причиной из `stderr.log`. Отдельного
   поиска квоты здесь больше нет: единственный источник этого вердикта — проверка 6.
...
- **`stderr.log`** — то, что CLI говорит вне протокола: паники, отказ на неизвестный флаг. В норме
  пуст, и его непустота сама по себе диагноз. Отсюда же берётся причина `FAIL`, когда события её
  не назвали.
- `raw.log` больше не существует. Под `--json` он был побайтовой копией `events.jsonl`, при этом
  ...
  командой `codex-bridge log <прогон>`, которая рендерит события по требованию.
