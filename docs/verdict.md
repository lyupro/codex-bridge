...
5. **Транспортные артефакты противоречат друг другу.** `status.json` с полем
   `stopped_on_deadline`, но без `stderr.log` рядом — невозможное сочетание: их пишет один и тот
   же вызов. Значит файл потерян уже после прогона, и `FAIL`. Проверка стоит выше `LIMIT`, потому
   что охраняет ровно тот откат к `raw.log`, которым `LIMIT` иначе воспользуется.
6. **Результат отсутствует или не заполнен.** Сначала в одной строке ищется одновременно
   ...
   предположением о лимите из текста ответа. Где именно ищется сигнал — см. «Транспорт и данные».
7. **Ненулевой exit при заполненном результате.** Это `FAIL`. Наличие JSON не отменяет сбой
   процесса.
8. **Красная verification у build.** `verify_passed === false` даёт `FAIL`, даже если изменения
...
9. **Объявленный исход build.** Исполнитель сам сообщает в обязательном поле `outcome`, сделана
...
10. **Покрытие scout.** Для нескольких извлечённых вопросов требуются отдельные ответы,
...
11. **Правки build вне scope.** Из delta дерева сначала вычитаются `environmentPaths`, затем
...
12. **Соответствие отчёта build фактической работе.** `changes[].file` сначала сравниваются с
...
13. **Успех.** Совпадение только с накопленной работой возвращает `OK` и `carried: true`; обычное
...
Поэтому потоки разъехались. `raw.log` по-прежнему содержит оба — он читается человеком, и резать
диагностику незачем. Но stderr дополнительно пишется в `stderr.log`, и сигнал квоты ищется только
там. Причина `FAIL` берётся из `raw.log` как раньше — неверная строка причины никого не заставляет
бросить работу.

**Этого мало, и это известно.** Живая проба 2026-08-05 (`2026-08-05_155023_plan15-deadline-probe`)
показала, что `codex exec` без флагов пишет в stderr весь человекочитаемый лог, включая вывод
инструментов: 206 013 байт из 206 086 ушли туда, и содержимое читаемых прогоном файлов оказалось в
`stderr.log`. Потоки у этого CLI делят вывод не по границе «транспорт против данных», поэтому
ложный `LIMIT` из цитаты по-прежнему возможен. Класс закрывается переходом на поток событий
(`codex exec --json`) — см. [Plan_16](plans/Plan_16-events-are-the-transport.md). Что уже работает
и подтверждено живьём — проверка 4: убитый по дедлайну прогон называет дедлайн и до `LIMIT` не
доходит.

Признак того, что прогон писал потоки раздельно, — **наличие самого `stderr.log`**, как признаком
объявленного исхода служит `schema.json`. Есть файл — квота ищется в нём; нет файла — прогон
архивный и судится по `raw.log` по-прежнему. Проверяется существование, а не размер: пустой файл
означает «stderr молчал», и это данные, а не отсутствие контракта.

У этого отката есть цена, и она закрыта проверкой 5. Откат предназначен архивным прогонам, но
сработал бы и на **новом**, у которого `stderr.log` пропал после прогона, — и дефект вернулся бы
молча, ровно там, где его чинили. Поэтому признаков два: `stderr.log` и поле `stopped_on_deadline`
в `status.json`. Их пишет один и тот же вызов, поэтому расхождение означает не архив, а порчу, и
такой прогон получает `FAIL`, а не откат к `raw.log`.
...
Цена решения названа заранее: если Codex CLI однажды напечатает ошибку квоты в stdout, прогон
получит `FAIL` вместо `LIMIT`. Это ошибка в безопасную сторону — `FAIL` зовёт посмотреть, тогда
как ложный `LIMIT` бросает доделанную работу.
...
невыполнимой задачи, получал `OK`) закрыта не здесь, а объявленным исходом: проверка 9 краснеет по
...
| `LIMIT` | Результата нет и stderr прогона содержит подтверждённый сигнал исчерпания квоты. | `3` |
