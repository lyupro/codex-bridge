...
изменения heartbeat превышает пять минут. Такой прогон нельзя закрывать через `sweep`: worker
...
`codex-bridge sweep` — ручной промежуточный шаг между адресным `stop` и автоматической проверкой
`markAbandoned()` в начале следующего прогона (шаг 5 launcher). Без аргумента он проходит всё
хранилище, с именем — один проект, и вызывает ту же проверку мёртвого pid, а не копирует её.
Закрываются только записи `state=running` с мёртвым pid; второй запуск ничего не меняет. `sweep`
не удаляет ни каталоги, ни транспортные файлы и перечисляет живые pid с устаревшим heartbeat с
готовой командой `codex-bridge stop <прогон>`.

В отличие от `stop`, `sweep` не снимает состояние рабочего дерева, поэтому в `abandoned_reason`
закрытых им прогонов списка файлов не будет. Это сознательный выбор: `stop` закрывает один
названный прогон и знает его репозиторий, а свип по всему хранилищу гонял бы `git` во всех
