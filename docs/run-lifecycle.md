...
2. `runCodex()` получает полный `task.md` через stdin. stdout и stderr потоково дописываются в
   `raw.log`, а stderr дополнительно — в `stderr.log`; у каждого файла свой предел в 256 MiB, и
   превышение обрезает файл, а не убивает прогон. Два файла нужны потому, что по `stderr.log`
   ищется сигнал исчерпания квоты: в общий поток попадает содержимое читаемых прогоном файлов, и
   цитата чужой ошибки красила прогон в `LIMIT` (см. `verdict.md`). `stderr.log` создаётся всегда,
   даже когда stderr промолчал. Если любой из двух файлов не пишется (нет прав, кончился диск),
