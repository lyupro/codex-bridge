...
report from memory. If an attaching call is killed by a time ceiling, repeat the identical command
once with `--no-wait`. This only checks state; it is never a final answer. If stdout contains the
ready reply, return it normally. If the call exits 4, return to the ordinary waiting call with the
same order id and without `--no-wait`. A real run takes 20-25 minutes, which is normal, not a hang.
