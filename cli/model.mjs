...
function profileSource(profile, role, userTier) {
  ...
  const source = model === effort ? model : `model: ${model}; effort: ${effort}`;
  const speed = profile.speed_source === 'config' ? 'config file'
    : role === 'build' && userTier ? `operator Codex config (${userTier})` : 'not pinned';
  const ignored = role === 'build' ? '' : ' (operator Codex config not read)';
  return `${source}; speed: ${speed}${ignored}`;
}
...
export async function model(argv = [], options = {}) {
  ...
  try {
    ...
    const userTier = readCodexUserTier(options);
    const rows = Object.entries(AGENTS).map(([agent, { role }]) => {
      ...
      return {
        ...
        speed: profile.speed || 'not pinned',
        source: profileSource(profile, role, userTier),
      };
    });
    return {
      exitCode: 0,
