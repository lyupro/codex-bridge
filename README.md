...
The current suite contains **880 automated tests: 879 passing and 1 skipped**.
...
| `model [list\|set\|unset]` | Show the model, effort and provenance of each role, with the config path; `model list` prints the live catalogue from Codex, hidden models included and marked; `model set <role> <model> [effort]` pins a role after checking the pair against that model's catalogue entry, and `model unset <role>` returns the role to whatever Codex chooses. The profile is machine-wide, shared by every project on this machine. |
